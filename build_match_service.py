...
class MatchService:
    ...
    def _sync_match_details_and_players(cls, db: Session, match: Match, scraper):
        ...
        for p_stat in detail_data.get("player_stats", []):
            existing_stat = db.query(PlayerMatchStat).filter(
                PlayerMatchStat.match_id == match.id,
                PlayerMatchStat.team_name == p_stat["team_name"],
                PlayerMatchStat.player_name == p_stat["player_name"]
            ).first()
            ...
            if not existing_stat:
                new_stat = PlayerMatchStat(
                    ...
                    minutes_played=0,
                    points=p_stat.get("points", 0), # 타점 또는 삼진
                    assists=0,
                    shots=p_stat.get("shots", 0),   # 타수 또는 투구이닝
                    extra_stats=extra_str,
                    is_override=False
                )
                db.add(new_stat)
            elif not existing_stat.is_override:
                existing_stat.points = p_stat.get("points", 0)
                existing_stat.shots = p_stat.get("shots", 0)
                existing_stat.extra_stats = extra_str
...
with open("app/services/match_service.py", "w", encoding="utf-8") as f:
    f.write(code)
print("app/services/match_service.py updated for baseball!")