...
class NpbOfficialScraper:
    ...
    def _scrape_yahoo_game_detail(self, game_id: str) -> Dict[str, Any]:
        ...
        # 선발 투수 정보 추출
        home_st_p = next((p for p in player_stats if p["team_name"] == home_name and p.get("extra_stats", {}).get("is_starter")), None)
        away_st_p = next((p for p in player_stats if p["team_name"] == away_name and p.get("extra_stats", {}).get("is_starter")), None)

        def make_npb_st_dict(p_obj):
            if not p_obj:
                return {}
            ex = p_obj.get("extra_stats", {})
            return {
                "name": p_obj.get("player_name", ""),
                "name_raw": ex.get("name_raw", p_obj.get("player_name", "")),
                "name_en": ex.get("name_raw", p_obj.get("player_name", "")),
                "throws": "우완",
        ...
        team_stats["starters"] = {
            "home": make_npb_st_dict(home_st_p),
            "away": make_npb_st_dict(away_st_p)
        }
    ...
    def scrape_game_detail(self, official_id: str) -> Dict[str, Any]:
        ...
        team_stats["starters"] = {
            "home": make_npb_st_dict2(home_st_p),
            "away": make_npb_st_dict2(away_st_p)
        }
