    ...
    def export_league_to_folder_structure(cls, db: Session, league_id: str = "MLB", start_date: str = "2026-09-01", end_date: str = "2026-09-07", base_dir: str = "exports"):
        ...
        for team_name in sorted(list(team_names)):
            ...
            for p_name, p_data in player_logs.items():
                ...
                if p_type == "PITCHER" or "P" in (p_data["position"] or ""):
                    # 투수 정밀 분석 파일
                    target_dir = pitchers_dir
                    p_file = f"{safe_p}.json"
                    
                    total_so = sum(g["extra_stats"].get("strikeouts", g.get("points", 0)) for g in p_data["games"])
                    total_er = sum(g["extra_stats"].get("er", 0) for g in p_data["games"])
                    ...
                    content = {
                        ...
                        "aggregated_pitching_stats": {
                            ...
                            "season_era": p_data["extra_stats"].get("era", "3.00"),
                            "season_whip": p_data["extra_stats"].get("whip", "1.10"),
...
with open("app/services/folder_export_service.py", "w", encoding="utf-8") as f:
    f.write(code)
print("app/services/folder_export_service.py updated for baseball!")